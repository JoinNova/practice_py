...
sub1='https://www.coupang.com/np/search?q=tv&channel=user&component=&eventCategory=SRP&trcid=&traid=&sorter=scoreDesc&minPrice=&maxPrice=&priceRange=&filterType=&listSize=36&filter=&isPriceRange=false&brand=&offerCondition=&rating=0&page='
sub2='&rocketAll=false'
...
#l = soup.find_all("div",class_='search-content search-content-with-feedback ')
   ...
   for _ in l:
       ...
       product.insert((i-1)*36*2+2*chk+1,_)
       chk+=1
...
#l = soup.find_all("div",class_='search-content search-content-with-feedback ')
   ...
   for _ in l:
       ...
       _='https://'+_     
       #product.insert(3*chk+2,_)
       product.insert((i-1)*36*3+3*chk+2,_)
       chk+=1
...
for i in range(len(product)//3):
    ...
    result+='"'+str(i+1)+'"'+','+'"'+product[i*3]+'"'+','+'"'+product[i*3+1]+'"'+','+'"'+product[i*3+2]+'"'+'\n'#+'"'+str(photo[i])+'"'+'\n'
file_ = open('18dec14_coupangTV.csv', 'w')
file_.write(result)
file_.close()
